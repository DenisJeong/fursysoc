...
class Template_Compiler_
{
    function _compile_template($tpl, $template_path, $compile_base, $compile_header)
    {
        ...
        // get template
        $source = '';
        if ($source_size = filesize($template_path)) {
            $fp_tpl = fopen($template_path, 'rb');
        ...
        $this->_save_result($compile_path, $compile_header, ' */ ' . $plugins . $size_of_top_loop . '?>' . $nl, $source);
    ...
    function _compile_statement($statement)
    {
        ...
        if ($match[1]) {
            switch ($match[2]) {
                ...
                case '?':
                    return $this->_compile_branch($src, 1, 0) !== 0 ? array(16, substr($statement, 1)) : 0;
                case ':':
                    return !strlen($src) || $this->_compile_branch($src, 1, 1) !== 0 ? array(16, substr($statement, 1)) : 0;
                case '' :
                    return $this->_compile_expression($src, 1, 1) !== 0 ? array(16, substr($statement, 1)) : 0;
                default :
                    return $this->_compile_expression($src, 1, 0) !== 0 ? array(16, substr($statement, 1)) : 0; // = @ :?
            }
        }
        switch ($match[2]) {
            case ''  :
                return (($xpr = $this->_compile_expression($src, 0, 1)) === 0) ? 0 : array(1, 'echo ' . $xpr);
            case '=' :
                return (($xpr = $this->_compile_expression($src, 0, 0)) === 0) ? 0 : array(1, 'echo ' . $xpr);
            case ':?':
                return (($xpr = $this->_compile_expression($src, 0, 0)) === 0) ? 0 : array(2, '}elseif(' . $xpr . '){'); // deprecated
            case '+' :
                return !strlen($src) || preg_match('/^[A-Z_a-z\x7f-\xff][\w\x7f-\xff]*$/', $src) ? array(8, $src) : 0;
            case '#' :
                return preg_match('/^[A-Z_a-z\x7f-\xff][\w\x7f-\xff]*$/', $src) ? array(4, '$this->print_("' . $src . '",$TPL_SCP,1);') : 0;
            case '@' :
    ...
    function _save_result($compile_path, $compile_header, $init_code, $source)
    {
        ...
        $source_size = strlen($compile_header) + strlen($init_code) + strlen($source) + 9;

        $source = $compile_header . str_pad($source_size, 9, '0', STR_PAD_LEFT) . $init_code . $source;

        $fp_cpl = fopen($compile_path, 'wb');
        if (false === $fp_cpl) {
            $this->report('Error #10', 'cannot write compiled file "<b>' . $compile_path . '</b>"');
            $this->exit_();
        }
        ...
        if (!$this->on_ms) @chmod($compile_path, $this->permission & ~0111);
    }
}
