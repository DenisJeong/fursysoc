...
class Template_
{
        ...
        while ($f = @$d->read()) {
            switch ($f) {
                ...
                case '..':
                    break;
                default :
                    @is_dir($f = $dir_path . '/' . $f) ? $this->_empty_out_recursively($f, 1) : @unlink($f);
            }
        }
    ...
    function _get_compile_path($rel_path)
    {
        ...
        if ($this->path_digest) {    // enable path_digest to use abs path for $compile_dir and rel path for $template_dir
...
function _template_notice_handler($type, $msg, $file, $line)
{
    ...
    switch ($type) {
        case E_NOTICE      :
            $msg = '<span style="font:12px tahoma,arial;color:green;background:white">Template_ Notice #1: ' . $msg . '</span>';
            break;
        case E_WARNING     :
        case E_USER_WARNING:
            $msg = '<b>Warning</b>: ' . $msg;
