...
$description = '[fursysocmall] '.$description;
...
if ($name and $phone and $email and $address) {
  ...
  try {
    ...
    $mail->Host = "smtp.works.naver.com"; // email 보낼때 사용할 서버를 지정
    $mail->SMTPAuth = true;       // SMTP 인증을 사용함
    $mail->Port = 587;            // email 보낼때 사용할 포트를 지정
    $mail->SMTPSecure = "tls";    // SSL을 사용함
    $mail->Username   = "fursysoc@fursysofficeconsultant.com"; // Gmail 계정
    $mail->Password   = "fursysoc34";      // 패스워드
     
    $mail->SetFrom($email); // 보내는 사람 email 주소와 표시될 이름 (표시될 이름은 생략가능)
    $mail->AddAddress('fursysoc@fursysofficeconsultant.com', '퍼시스 오피스 컨설턴트'); // 받을 사람 email 주소와 표시될 이름 (표시될 이름은 생략가능)
